...
const SubCategoryForm = ({ data, productId, options }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.info("Product sub - category updated")
      toggleEdit()
      router.refresh()
    } catch (error) {
  ...
  const selectedOptions = options.find((option) => (option.value === data.subCategory));
  return (
